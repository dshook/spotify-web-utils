...
export default class SpotifyApp{
  ...
  async authenticate(req, res){
    ...
    if(state && code){
      ...
      res.redirect(req.baseUrl + state + delimiter + 'code=' + code);
      return;
    }
  ...
  async scrape(req, res){
    ...
    try{
      var spotifyApi = await this.getSpotifyApi(code, req);
  ...
  async coverify(req, res){
    ...
    if(!code){
      console.log('Redirecting to Auth'); 
      res.redirect(req.baseUrl + '/auth?' + querystring.stringify({returnUrl: req.path + '?playlist=' + playlistName}));
      return;
    }
